...
predBiospear= function(res, method, newdata, randomOS){
  ...
  if (randomOS == T){
    newdata$OS = sample(c(1:1825), size = nrow(newdata), replace = T)
    newdata$OS.Event = sample(c(0,1), size = nrow(newdata), replace = T)
  }
